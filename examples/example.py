...
sim_dir = join(data_dir, "GrandEventADetailed2") # will be copied from $PROJECT to $TMP, scaled_Evet* can be stored there since it will be overwritten for each event

# ATTENTION: set here the path to the temporary storage at evry core, for testing set to test_dir created
save_dir=data_dir = join(root_dir, "examples", "test_temp") # "$TMP" at core
if not os.path.exists(save_dir): # later this is nit necessary with $TMP
...
#### Then, move it after antenna and take RM into the loop
   ...
   ### ENERGY
   pp=event["decay"][1][1] # momentum vector, second decay product: event["decay"][2][1] 
   ...
   PID= float(event["decay"][1][0])
   print PID
   el_list=[22.0,11.0,15.0,13.0] #'22.0':'gamma', '11.0': 'electron',  '15.0':'tau', '13.0':'muon'
   if PID in el_list:
       primary="electron"
   else: # pion-like
       primary="pion"
...
####   
   ...
   out_dir = join(save_dir, "InterpolatedSignals", str(event["tag"])) # will be created in $TMP, will be deleted after each event 
   print out_dir
