...
tmp_dir= join(root_dir, "examples", "test_temp") # "$TMP" at core
...
for event in EventIterator(str(sys.argv[1])):#"events-flat.json"): json files contains a list of events which shall run on one node
   ...
   #j=j+1
        ...
        if subshowers==0:
                ...
                particle=int(np.argmax(ep_array) +1) # not forget about the inital neutrino
                PID= float(event["decay"][int(np.argmax(ep_array) +1)][0])
                el_list=[22.0,11.0,15.0,13.0, 111.0] #'22.0':'gamma', '11.0': 'electron',  '15.0':'tau', '13.0':'muon', '111.0': 'pi0'
                if PID in el_list:
                    primary="electron"
                else: # pion-like
                    primary="pion"
